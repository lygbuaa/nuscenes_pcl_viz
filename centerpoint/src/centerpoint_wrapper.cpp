...
std::shared_ptr<SampleCenterPoint> g_sample_ptr_ = nullptr;
...
bool load_centerpoint_model(const std::string& trt_model_path)
{
    ...
    g_sample_ptr_ = std::make_shared<SampleCenterPoint>(params);
    sample::gLogInfo << "loading a GPU inference engine for CenterPoint" << std::endl;

    if(!g_sample_ptr_->LoadTrtEngine(trt_model_path))
    {
        sample::gLogError << "LoadTrtEngine failed!" << std::endl;
        return false;
    }
...
bool infer_centerpoint_model(float* points, const int pointNum, std::vector<Box>& predResult)
{
    ...
    {
        sample::gLogError << "SampleCenterPoint null!" << std::endl;
        return false;
    }

    if(!g_sample_ptr_->infer(points, pointNum, predResult))
    {
        sample::gLogError << "SampleCenterPoint infer failed!" << std::endl;
        return false;
    }
